...
def has_local_catalog_or_price_list_price(
    ...
    if catalog_hit and catalog_hit.score >= LOCAL_MATCH_THRESHOLD:
        if isinstance(catalog_hit.payload, CatalogItem):
            item = catalog_hit.payload
            if item.cost is not None or item.price is not None:
                if is_relevant_match(tz_item, catalog_hit.name, score=catalog_hit.score):
    ...
    if price_hit and price_hit.score >= LOCAL_MATCH_THRESHOLD:
        if isinstance(price_hit.payload, PriceListItem):
            if price_hit.payload.price is not None:
                if is_relevant_match(tz_item, price_hit.name, score=price_hit.score):
                    return True
